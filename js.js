// ========================================
// START SCRIPT EXECUTION CHECK
// ========================================
console.log("--- app.js: Script parsing started ---");

// ========================================
// Firebase Initialization
// ========================================
import { initializeApp } from "https://www.gstatic.com/firebasejs/9.15.0/firebase-app.js"; // Using CDN for simplicity
import {
    getAuth,
    ...
    onAuthStateChanged,
    signOut
} from "https://www.gstatic.com/firebasejs/9.15.0/firebase-auth.js"; // Using CDN

// Your web app's Firebase configuration (Ensure this is correct!)
const firebaseConfig = {
    apiKey: "AIzaSyDv-iR_HNAIVgwxPsQYeLQAl2eMasqcoSk", // Use your actual key
    authDomain: "twitterpro-65d42.firebaseapp.com",
    projectId: "twitterpro-65d42",
    storageBucket: "twitterpro-65d42.appspot.com", // Ensure domain matches console
    messagingSenderId: "1011809439131",
    appId: "1:1011809439131:web:99e7f9e1f73b070c9b253e",
    measurementId: "G-PV33CCWHCT" // Optional
};

// Initialize Firebase within a try...catch
let app;
let auth;
try {
...
const userNavContainer = document.getElementById('user-navigation-links'); // Add this ID to the <ul> in the header
const loggedInUserInfo = document.getElementById('logged-in-user-info'); // Add this ID to a container for user info/logout
const signupButton = document.getElementById('signup-btn'); // Ensure you have a signup button in the header
const loginButton = document.getElementById('login-btn'); // Ensure you have a login button in the header
const logoutButton = document.getElementById('logout-btn'); // Ensure you have a logout button in the header
...
if (loginForm && auth) {
    ...
    loginForm.addEventListener('submit', async (e) => {
        ...
        try {
            console.log("--- app.js: Attempting Firebase sign in... ---");
...
if (registerForm && auth) {
    ...
    registerForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        console.log("--- app.js: Register form submitted ---");
        ...
        try {
            console.log("--- app.js: Attempting Firebase user creation... ---");
...
// ========================================
// Auth State Listener - Updates UI across all pages
// ========================================
if (auth) {
    ...
    console.error("--- app.js: Auth service not available, cannot set listener. ---");
}
...
function updateNavigation(user) {
    ...
    if (user) {
        // User is logged in
        if (loggedInUserInfo) {
            loggedInUserInfo.textContent = `Hello, ${user.email}`; // Display username or email
            loggedInUserInfo.style.display = 'block';
        }

        // Hide login and signup buttons when logged in
        if (userNavContainer) {
            userNavContainer.style.display = 'none'; // Hide navigation links like login and register
        }
        ...
        if (userNavContainer) {
            userNavContainer.style.display = 'block'; // Show login/register navigation links
        }
        ...
        // Show signup and login buttons when logged out
        if (signupButton) {
